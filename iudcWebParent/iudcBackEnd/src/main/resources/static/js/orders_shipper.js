...
function sendRequestToUpdateOrderStatus(button) {
	...
	$.ajax({
		...
		showMessageModal("Orden actualizada correctamente");
		updateStatusIconColor(response.orderId, response.status);
		
		console.log(response);
	}).fail(function(err) {
		showMessageModal("Error al actualizar el estatus de la orden");
	})
}
...
function showUpdateConfirmModal(link) {
	...
	confirmText.text(" Estas seguro que desdeas actualizar el estaudos de la orden ID #" + orderId
					 + " a " + status + "?");
