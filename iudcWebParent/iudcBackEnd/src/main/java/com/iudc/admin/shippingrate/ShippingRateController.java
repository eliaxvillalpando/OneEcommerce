...
public class ShippingRateController {
	...
	public String saveRate(ShippingRate rate, RedirectAttributes ra) {
		try {
			service.save(rate);
			ra.addFlashAttribute("message", "La tarifa de envio ha sido actualizada.");
	...
	public String editRate(@PathVariable(name = "id") Integer id,
		...
		try {
			...
			model.addAttribute("pageTitle", "Editar tarifa de envio (ID: " + id + ")");
	...
	public String updateCODSupport(@PathVariable(name = "id") Integer id,
		...
		try {
			...
			ra.addFlashAttribute("message", "Pago a credito para la tarifa de envio con ID " + id + " ha sido actualizada.");
		} catch (ShippingRateNotFoundException ex) {
			ra.addFlashAttribute("message", ex.getMessage());
		}
		return defaultRedirectURL;
	...
	public String deleteRate(@PathVariable(name = "id") Integer id,
		...
		try {
			service.delete(id);
			ra.addFlashAttribute("message", "La tarifa de envio con ID " + id + " ha sido eliminada.");
		} catch (ShippingRateNotFoundException ex) {
			ra.addFlashAttribute("message", ex.getMessage());
		}
		return defaultRedirectURL;
