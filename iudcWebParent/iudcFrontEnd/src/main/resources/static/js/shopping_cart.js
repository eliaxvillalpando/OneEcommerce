...
function decreaseQuantity(link) {
	...
	if (newQuantity > 0) {
		...
		showWarningModal('La cantidad minima es 1');
	}	
}
...
function increaseQuantity(link) {
		...
		if (newQuantity <= 1000) {
			...
			showWarningModal('La cantidad maxima es 1000');
		}	
}
...
function updateQuantity(productId, quantity) {
	...
	$.ajax({
		...
		showErrorModal("Error al actualizar la cantidad del producto.");
	});	
}
...
function removeProduct(link) {
	...
	$.ajax({
		...
		showModalDialog("Carrito de compras", response);
            ...
            showErrorModal("Error al qutiar el producto.");
	});				
}
